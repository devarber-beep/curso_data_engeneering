...
session_metrics AS (
    SELECT 
        ...
        COUNT(DISTINCT CASE WHEN e.name = 'add_to_cart' THEN e.EVENT_ID ELSE NULL END) AS ADD_TO_CART_EVENTS,
        COUNT(DISTINCT CASE WHEN e.name = 'checkout' THEN e.EVENT_ID ELSE NULL END) AS CHECKOUT_EVENTS,
        COUNT(DISTINCT CASE WHEN e.name = 'package_shipped' THEN e.EVENT_ID ELSE NULL END) AS PACKAGE_SHIPPED_EVENTS,
        COUNT(DISTINCT CASE WHEN e.name = 'page_view' THEN e.EVENT_ID ELSE NULL END) AS PAGE_VIEW_EVENTS
    FROM events e
